...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  numClasses = size(model.hid_to_class,1);
  data.targets
  groundTruth = data.targets;
  
  z1 = model.input_to_hid*data.inputs;
  h1 = logistic(z1);
  z2 = model.hid_to_class * h1;
  z2 = z2 - repmat(max(z2), numClasses, 1);
  exp_z2 = exp(z2);
  sum_exp_z2 = sum(exp_z2,1);
  softmax = exp_z2 ./ repmat(sum_exp_z2, numClasses, 1);
  
  % backward pass
  error_deriv = -(groundTruth - softmax)/numCases;
  ret.hid_to_class = error_deriv * h1' + wd_coefficient * model.hid_to_class;
  
  net2_deriv = (model.hid_to_class' * error_deriv) .* h1 .* (1 - h1);
  ret.input_to_hid = net2_deriv * data.inputs' + wd_coefficient * model.input_to_hid;
